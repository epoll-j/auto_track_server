import {
  ...
  function TimelineModal({ show, params, onOk, onCancel }: ParamsModalProps) {
    ...
    return (
      <Modal title="Activity Timeline" open={show} onOk={onOk} onCancel={onCancel}>
        {loading ? <Spin /> :
          <Timeline
            className="!mt-4 w-full max-h-[60vh] overflow-y-auto"
            items={(data || []).map((item) => (
              {
                ...
                children: (
                  <div className="flex flex-col">
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      <Popover content={
                        <pre><code>{JSON.stringify(JSON.parse(item.trackParams || '{}'), null, 2)}</code></pre>
                      } title="Params">
                        <InfoCircleOutlined />
                      </Popover>
                    </div>
